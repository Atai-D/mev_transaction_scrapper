...
class Bot extends TelegramBotSetup {
  ...
  async sendMessage(text, chatID, parseMode, disableNotification) {
    try {
      const result = await this.api(
        `/sendMessage?text=${text}&chat_id=${
          chatID ? chatID : this.dcid
        }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
  ...
  async sendPhoto(img, caption, chatID, parseMode, disableNotification) {
    try {
      if (img.startsWith('#')) {
        ...
        const result = await this.api(
          `/sendPhoto?caption=${caption ? caption : ''}&chat_id=${
            chatID ? chatID : this.dcid
          }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
            disableNotification ? disableNotification : false
          }`,
        ...
        const result = await this.api(
          `/sendPhoto?photo=${img}&caption=${caption ? caption : ''}&chat_id=${
            chatID ? chatID : this.dcid
          }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
            disableNotification ? disableNotification : false
          }`,
  ...
  async sendAudio(audio, caption, chatID, parseMode, disableNotification) {
    try {
      if (audio.startsWith('#')) {
        ...
        const result = await this.api(
          `/sendAudio?caption=${caption ? caption : ''}&chat_id=${
            chatID ? chatID : this.dcid
          }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
            disableNotification ? disableNotification : false
          }`,
        ...
        const result = await this.api(
          `/sendAudio?audio=${audio}&caption=${
            caption ? caption : ''
          }&chat_id=${chatID ? chatID : this.dcid}&parse_mode=${
  ...
  async sendVoice(voice, caption, chatID, parseMode, disableNotification) {
    try {
      if (voice.startsWith('#')) {
        ...
        const result = await this.api(
          `/sendVoice?caption=${caption ? caption : ''}&chat_id=${
            chatID ? chatID : this.dcid
          }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
            disableNotification ? disableNotification : false
          }`,
  ...
  async sendVideo(video, caption, chatID, parseMode, disableNotification) {
    try {
      if (video.startsWith('#')) {
        ...
        const result = await this.api(
          `/sendVideo?caption=${caption ? caption : ''}&chat_id=${
            chatID ? chatID : this.dcid
          }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
            disableNotification ? disableNotification : false
          }`,
  ...
  async sendFile(doc, caption, chatID, parseMode, disableNotification) {
    try {
      if (doc.startsWith('#')) {
        ...
        const result = await this.api(
          `/sendDocument?caption=${caption ? caption : ''}&chat_id=${
            chatID ? chatID : this.dcid
          }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
            disableNotification ? disableNotification : false
          }`,
        ...
        const result = await this.api(
          `/sendDocument?document=${doc}&caption=${
            caption ? caption : ''
          }&chat_id=${chatID ? chatID : this.dcid}&parse_mode=${
            parseMode ? parseMode : 'html'
          }&disable_notification=${
  ...
  async sendContact(
    ...
    try {
      const result = await this.api(
        `/sendContact?phone_number=${phoneNumber}&first_name=${firstName}&last_name=${
          lastName ? lastName : ''
        }&chat_id=${chatID ? chatID : this.dcid}&parse_mode=${
          parseMode ? parseMode : 'html'
        }&disable_notification=${
  ...
  async sendLocation(
    ...
    try {
      const result = await this.api(
        `/sendLocation?latitude=${latitude}&longitude=${longitude}&chat_id=${
          chatID ? chatID : this.dcid
        }&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${
          disableNotification ? disableNotification : false
        }`,
  ...
  async sendPoll(question, options, config, chatID, disableNotification) {
    try {
      let url = `/sendPoll?question=${question}&options=${JSON.stringify(
        options
      )}&chat_id=${chatID ? chatID : this.dcid}&disable_notification=${
        disableNotification ? disableNotification : false
      }`;
      for (let i in config) {
        url += `&${i}=${config[i]}`;
